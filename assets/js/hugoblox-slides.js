...
const revealThemeLink = document.querySelector('link[href*="/dist/theme/"][href$=".min.css"]');
const chromaDarkThemeLink = document.querySelector('link[href*="/css/libs/chroma/"][href$=".min.css"]:not(#chroma-light-theme)');
const chromaLightThemeLink = document.querySelector("#chroma-light-theme");
const revealThemeStorageKey = "hugoblox-slides-reveal-theme";

const getRevealTheme = () => {
  const match = revealThemeLink?.href.match(/\/dist\/theme\/([^/]+)\.min\.css/);
  return match?.[1] || "black";
};

const setRevealTheme = (theme) => {
  if (!revealThemeLink) return;
  revealThemeLink.href = revealThemeLink.href.replace(/\/dist\/theme\/[^/]+\.min\.css/, `/dist/theme/${theme}.min.css`);
  document.documentElement.dataset.revealTheme = theme;
  localStorage.setItem(revealThemeStorageKey, theme);
};

const setChromaTheme = (theme) => {
  if (chromaDarkThemeLink) {
    chromaDarkThemeLink.disabled = theme === "white";
  }

  if (chromaLightThemeLink) {
    chromaLightThemeLink.disabled = theme !== "white";
  }
};

const setPresentationTheme = (theme) => {
...
const storedRevealTheme = localStorage.getItem(revealThemeStorageKey);
if (storedRevealTheme === "black" || storedRevealTheme === "white") {
  setPresentationTheme(storedRevealTheme);
} else {
  document.documentElement.dataset.revealTheme = getRevealTheme();
  setChromaTheme(getRevealTheme());
}

document.addEventListener("keydown", (event) => {
  if (event.key.toLowerCase() !== "t" || event.altKey || event.ctrlKey || event.metaKey) return;
  if (["INPUT", "TEXTAREA", "SELECT"].includes(event.target?.tagName) || event.target?.isContentEditable) return;

  setPresentationTheme(getRevealTheme() === "black" ? "white" : "black");
});
...
if (slides.diagram) {
  const getDefaultMermaidOptions = (theme) => ({
    theme: "base",
    themeVariables: {
      fontFamily: "monospace",
      fontSize: "24px",
      primaryColor: theme === "white" ? "#fff7ed" : "#282a36",
      primaryBorderColor: "#ffb86c",
      primaryTextColor: theme === "white" ? "#1f2937" : "#f8f8f2",
      lineColor: "#ffb86c",
      edgeLabelBackground: theme === "white" ? "#ffffff" : "#282a36",
      tertiaryColor: theme === "white" ? "#ffffff" : "#282a36",
    },
    flowchart: {
      htmlLabels: false,
      nodeSpacing: 45,
  ...
  const getMermaidOptions = (theme) => {
    const defaultMermaidOptions = getDefaultMermaidOptions(theme);

    // mermaid: front matter configuration can be used to override these defaults.
    if (typeof slides.diagram_options === "undefined") {
      return defaultMermaidOptions;
    }

    return {
      ...defaultMermaidOptions,
      ...slides.diagram_options,
      themeVariables: {
        ...defaultMermaidOptions.themeVariables,
        ...slides.diagram_options.themeVariables,
      },
      flowchart: {
        ...defaultMermaidOptions.flowchart,
        ...slides.diagram_options.flowchart,
      },
    };
  };

  // `startOnLoad` must be false since diagrams are lazily rendered.
  const mermaidOptions = getMermaidOptions(getRevealTheme());
  ...
  const renderMermaidDiagrams = function renderMermaidDiagrams(event) {
    ...
    mermaidDivs.forEach((mermaidDiv, i) => {
      ...
      const graphDefinition = mermaidDiv.dataset.graphDefinition || mermaidDiv.textContent;
      mermaidDiv.dataset.graphDefinition = graphDefinition;
  ...
  window.updateMermaidTheme = (theme) => {
    const nextMermaidOptions = getMermaidOptions(theme);
    nextMermaidOptions.startOnLoad = false;
    mermaid.initialize(nextMermaidOptions);

    document.querySelectorAll(".mermaid.done").forEach((mermaidDiv) => {
      mermaidDiv.innerHTML = mermaidDiv.dataset.graphDefinition || mermaidDiv.textContent;
      mermaidDiv.classList.remove("done");
    });

    renderMermaidDiagrams({ currentSlide: document });
  };
